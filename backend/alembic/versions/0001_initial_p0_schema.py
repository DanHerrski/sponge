...
def upgrade() -> None:
    ...
    node_type = sa.Enum(
        "idea", "story", "framework", "definition", "evidence", "theme",
        name="node_type", create_type=True,
    )
    edge_type = sa.Enum(
        "supports", "example_of", "expands_on", "related_to", "contradicts",
        name="edge_type", create_type=True,
    )
    nugget_type = sa.Enum("idea", "story", "framework", name="nugget_type", create_type=True)
    nugget_status = sa.Enum("new", "explored", "parked", name="nugget_status", create_type=True)
    source_type = sa.Enum("chat", "upload", name="source_type", create_type=True)
    confidence_level = sa.Enum("low", "med", "high", name="confidence_level", create_type=True)
    ...
    op.create_index("ix_chat_turns_session_turn", "chat_turns", ["session_id", "turn_number"])
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("session_id", UUID(as_uuid=True), sa.ForeignKey("sessions.id", ondelete="CASCADE"), nullable=False),
        sa.Column("source_id", UUID(as_uuid=True), sa.ForeignKey("nodes.id", ondelete="CASCADE"), nullable=False),
        sa.Column("target_id", UUID(as_uuid=True), sa.ForeignKey("nodes.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_index("ix_edges_session_type", "edges", ["session_id", "edge_type"])

    # --- nuggets ---
    op.create_table(
        "nuggets",
        sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("node_id", UUID(as_uuid=True), sa.ForeignKey("nodes.id", ondelete="CASCADE"), nullable=False, unique=True),
        ...
        sa.Column("status", nugget_status, nullable=False, server_default="new"),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
    )
    op.create_index("ix_nuggets_node", "nuggets", ["node_id"], unique=True)
